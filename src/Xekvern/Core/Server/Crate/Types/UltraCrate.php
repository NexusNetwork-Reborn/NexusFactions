...
class UltraCrate extends Crate
{
    ...
    public function __construct(Position $position)
    {
        parent::__construct(self::ULTRA, $position, [
            ...
            new Reward("Corrupted King Boss Egg", (new BossEgg("CorruptedKing"))->getItemForm(), function(NexusPlayer $player): void {
                $player->getInventory()->addItem((new BossEgg("CorruptedKing"))->getItemForm());
            }, 100),
            ...
            new Reward("Cosmetic Shard", (new CosmeticShard(random_int(0, 1), random_int(0, 13)))->getItemForm(), function (NexusPlayer $player): void {
                $items = [
                    (new CosmeticShard(random_int(0, 1), random_int(0, 13)))->getItemForm(),
                ];
                foreach ($items as $item) {
                    if ($player->getInventory()->canAddItem($item)) {
                        $player->getInventory()->addItem($item);
            ...
            new Reward("Coal Generator", (new OreGenerator(WorldHandler::getGeneratorColorById(GeneratorId::COAL, false)))->getItemForm()->setCount(1), function (NexusPlayer $player): void {
                $items = [
                    (new OreGenerator(WorldHandler::getGeneratorColorById(GeneratorId::COAL, false)))->getItemForm()->setCount(1),
                ];
                foreach ($items as $item) {
                    if ($player->getInventory()->canAddItem($item)) {
                        $player->getInventory()->addItem($item);
            ...
            new Reward("Ultra Tag", VanillaItems::NAME_TAG()->setCustomName(TextFormat::RESET . TextFormat::BOLD . TextFormat::RED . "Ultra" . TextFormat::RESET . TextFormat::GRAY . " Tag"), function (NexusPlayer $player): void {
